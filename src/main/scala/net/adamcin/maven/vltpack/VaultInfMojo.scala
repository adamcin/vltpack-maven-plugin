...
import com.day.jcr.vault.fs.api.{WorkspaceFilter, PathFilterSet}
...
class VaultInfMojo
  ...
  override def execute() {
    super.execute()
    generateConfigXml()
    val filter = generateFilterXml()
    val props = generatePropertiesXml()
    generateDefinition(filter, props)
  }
  ...
  def generateFilterXml(): WorkspaceFilter = {
  ...
  def generatePropertiesXml(): scala.collection.Map[String, String] = {
    ...
    if (!props.containsKey("createdBy")) {
      props.setProperty("createdBy", user)
    }
  ...
  def generateDefinition(filter: WorkspaceFilter, props: scala.collection.Map[String, String]) {
